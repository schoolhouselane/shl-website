    ...
    >
      ...
      <div className="flex flex-col gap-8 md:hidden">
        ...
        <div className="flex flex-col">
          {stats.map((s) => (
            <div key={s.label} className="border-b border-[#1e1e20] py-4 flex items-center justify-between">
              <p className="font-light text-[11px] uppercase tracking-widest text-[#1e1e20]">{s.label}</p>
              <p className="font-black text-[36px] leading-none text-[#1e1e20] uppercase">{s.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-6">
          <h3 className="font-black text-[42px] leading-[0.9] tracking-[-1.5px] uppercase text-[#1e1e20]">
            LET&apos;S BUILD<br />SOMETHING<br />REMARKABLE.
          </h3>
          <p className="text-[14px] leading-relaxed text-[#1e1e20] max-w-[300px]">
            15-minute strategy call. Let&apos;s discuss real ideas for your brand and what growth actually looks like for you.
          </p>
          <Link
            href="/contact"
            className="btn-cta self-start inline-flex items-center gap-2 border border-[#1e1e20] bg-white rounded-full px-5 py-3 text-[13px] font-semibold uppercase tracking-wider text-[#1e1e20] whitespace-nowrap"
          >
            Let&apos;s talk
