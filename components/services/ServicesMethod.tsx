...
export default function ServicesMethod() {
  return (
    <section className="bg-[#f5f3ef] px-4 md:px-6 lg:px-[90px] py-[60px] md:py-[80px] lg:py-[120px] flex flex-col gap-[40px] lg:gap-[80px]">
      ...
      {/* Steps — horizontal scroll on mobile, 5-col grid on tablet+ */}
      <div className="flex overflow-x-auto scrollbar-hide md:overflow-visible md:grid md:grid-cols-5">
        {steps.map((step, i) => (
          <div key={step.title} className={`flex-shrink-0 w-[200px] md:w-auto flex flex-col gap-[12px] items-start justify-start px-[20px] md:px-[24px] lg:px-[40px] py-[32px] md:py-[12px] lg:py-[60px] ${i < steps.length - 1 ? 'border-r border-[#1e1e20]' : ''}`}>
